...
def unet_no_skip_model_3d(input_shape, pool_size=(2, 2, 2), n_labels=1, deconvolution=False,
                  depth=4, n_base_filters=32, batch_normalization=False, activation_name="sigmoid"):
    K.set_image_data_format("channels_first")
    """
    Builds the 3D UNet Keras model.f
    ...
    for layer_depth in range(depth-2, -1, -1):
        ...
        # current_layer = create_convolution_block(n_filters=levels[layer_depth][1]._keras_shape[1], layer_depth=num_layer,
                                                 # input_layer=concat, batch_normalization=batch_normalization)
        current_layer = create_convolution_block(n_filters=levels[layer_depth][1]._keras_shape[1], layer_depth=num_layer,
                                                 input_layer=up-convolution, batch_normalization=batch_normalization)
        num_layer += 1
        current_layer = create_convolution_block(n_filters=levels[layer_depth][1]._keras_shape[1], layer_depth=num_layer,
                                                 input_layer=current_layer,
                                                 batch_normalization=batch_normalization)
...
def unetplusplus_model_3d(input_shape, pool_size=(2, 2, 2), n_labels=1, deconvolution=False,
                  depth=4, n_base_filters=32, batch_normalization=False, activation_name="sigmoid"):
    """
    Builds the 3D UNet++ Keras model.f
    :param metrics: List metrics to be calculated during model training (default is dice coefficient).
...
def super_unet_model_3d(input_shape, pool_size=(2, 2, 2), n_labels=1, deconvolution=False,
    ...
    for layer_depth in range(depth):
        ...
        if layer_depth < depth - 1:
            # current_layer = MaxPooling3D(pool_size=pool_size)(layer2)
            current_layer = Conv3D(n_base_filters*(2**layer_depth), (3, 3, 3), padding='same', strides=pool_size, name="depth_"+str(num_layer)+"_downsampling")(layer2)
            levels.append([layer1, layer2, current_layer])
        else:
            current_layer = layer2
            levels.append([layer1, layer2])
...
# 3D Dense-UNet:https://github.com/xmengli999/H-DenseUNet
...
def denseunet_3d(args):
    ...
    input2d = Lambda(slice1d, arguments={'h1': 0, 'h2': 2})(img_input)
    single = Lambda(slice1d, arguments={'h1':0, 'h2':1})(img_input)
    input2d = concatenate([single, input2d], axis=3)
    for i in xrange(args.input_cols - 2):
        input2d_tmp = Lambda(slice1d, arguments={'h1': i, 'h2': i + 3})(img_input)
        input2d = concatenate([input2d, input2d_tmp], axis=0)
        if i == args.input_cols - 3:
            final1 = Lambda(slice1d, arguments={'h1': args.input_cols-2, 'h2': args.input_cols})(img_input)
            final2 = Lambda(slice1d, arguments={'h1': args.input_cols-1, 'h2': args.input_cols})(img_input)
