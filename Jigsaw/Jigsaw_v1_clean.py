...
def randrot(data,permutation_set,batch_size,exp_choice):
    ...
    while True:
        ...
        for i,image in enumerate(batch_bag):
            ...
            temp = temp[permutation_bag[i]]
            temp = temp.reshape(2,2,2,int(H/2),int(W/2),int(D/2)).transpose(0,3,1,4,2,5).reshape(1,1,H,W,D)
        ...
        if exp_choice == 'encoder':
            yield(x,permutation_label)
        else:
            yield (x, [ori_bag, permutation_label])
...
if options.clc_head == 'conv':
    ...
    cls_output = Dense(1000, activation="softmax", name='cls_output')(cls_fc)
    ...
    cls_output = Dense(1000, activation="softmax", name='cls_output')(encoder_out)
else:
    # encoder_out = Flatten()(encoder_out)
    encoder_out = GlobalAveragePooling3D()(encoder_out)
    encoder_out = Dense(2048,activation="relu", name="clc_dense_1")(encoder_out)    
...
# train_data_gen = randrotplusplus(x_train,4,3,config.batch_size)
# validation_data_gen = randrotplusplus(x_valid, 4,3,config.batch_size)

train_data_gen = randrot(x_train, permutation_all, config.batch_size, options.exp_part)
validation_data_gen = randrot(x_valid, permutation_all, config.batch_size, options.exp_part)
