...
parser.add_option('--num_gpu', dest="num_gpu", type=int, default=None,  help='use multiple gpu?')
...
with tf.device('/cpu:0'):
    dis = keras.models.Model(inputs=[real_inp, fake_inp, disarrange_inp], 
                            outputs=[real_out, fake_out, validity_interpolated])
    
if options.num_gpu is not None:
    dis = multi_gpu_model(dis,gpus=options.num_gpu)
   
dis.compile(optimizer=Adam(lr=config.d_lr),
...
if options.num_gpu is not None:
    gen = multi_gpu_model(gen,gpus=options.num_gpu)
    dis_model = multi_gpu_model(dis_model,gpus=options.num_gpu)
    gen_base = multi_gpu_model(gen_base,gpus=options.num_gpu)

if config.init == 'pre-trained':
    gen.compile(optimizer=Adam(lr=0.0001),
